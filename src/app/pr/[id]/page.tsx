...
import type { FileResult, FileIndicator, ScanResult, RiskLevel } from "@/types";
...
const SIGNAL_META: Record<string, { label: string; desc: string; sev: SignalSev; cwe?: string; fix?: string }> = {
  // ── Security signals ──────────────────────────────────────────────────────────
  "hardcoded-secret":       { label: "Hardcoded Secret",          sev: "critical", cwe: "CWE-798",  fix: "Move to process.env or a secrets manager; rotate the exposed credential immediately.",
    desc: "An API key, password, token, or private key is embedded as a string literal. Any attacker with read access to the repo has the credential." },
  "high-entropy-secret":    { label: "High-Entropy String",       sev: "critical", cwe: "CWE-798",  fix: "Inspect the flagged string — if it is a credential, move it to an environment variable.",
    desc: "A high-randomness string (>4.2 bits/char Shannon entropy) in a credential context that doesn't match known key formats — likely a novel secret type." },
  "sql-injection":          { label: "SQL Injection",             sev: "critical", cwe: "CWE-89",   fix: "Replace string interpolation with parameterised queries ($1 placeholders or ORM methods).",
    desc: "User input is concatenated directly into a SQL string. An attacker can terminate the query and run arbitrary SQL — read, modify, or delete any data." },
  "xss":                    { label: "Cross-Site Scripting",      sev: "critical", cwe: "CWE-79",   fix: "Sanitize with DOMPurify before writing to innerHTML, or use a framework that auto-escapes.",
    desc: "Unsanitized content is written to the DOM via innerHTML or dangerouslySetInnerHTML. An attacker can inject scripts that run in the victim's browser." },
  "eval-exec":              { label: "Eval / Exec (RCE Risk)",    sev: "critical", cwe: "CWE-95",   fix: "Refactor to avoid dynamic code execution; use data-driven logic or a safe expression sandbox.",
    desc: "eval(), exec(), or Function() is called on a variable. If user input reaches this path, an attacker can execute arbitrary code on the server." },
  "ssrf":                   { label: "Server-Side Request Forgery", sev: "critical", cwe: "CWE-918", fix: "Parse the URL and verify the hostname against an explicit allowlist before fetching.",
    desc: "User-supplied input is used as a fetch/request URL without validation. An attacker can make the server probe internal services or cloud metadata endpoints." },
  "path-traversal":         { label: "Path Traversal",            sev: "critical", cwe: "CWE-22",   fix: "Call path.resolve() then verify the result starts with the expected base directory.",
    desc: "A file path is built from user input without canonicalization. An attacker can use ../ sequences to read or write arbitrary files on the server." },
  "insecure-deserialization": { label: "Insecure Deserialization", sev: "critical", cwe: "CWE-502", fix: "Replace node-serialize/pickle.loads with JSON.parse + schema validation.",
    desc: "Untrusted data is passed to a deserializer that can instantiate arbitrary objects. Can lead to remote code execution via crafted payloads." },
  "jwt-none-alg":           { label: "JWT None Algorithm",        sev: "critical", cwe: "CWE-347",  fix: "Whitelist only HS256/RS256; upgrade PyJWT to ≥2.8.0.",
    desc: "The JWT library is configured to accept the 'none' algorithm, which requires no signature. Any attacker can forge a valid token for any user." },
  "xxe":                    { label: "XML External Entity (XXE)", sev: "critical", cwe: "CWE-611",  fix: "Disable external entity processing: set feature 'http://xml.org/sax/features/external-general-entities' to false.",
    desc: "XML is parsed with external entity expansion enabled. An attacker can read local files or make internal network requests via crafted XML input." },
  "insecure-file-upload":   { label: "Insecure File Upload",      sev: "high",     cwe: "CWE-434",  fix: "Validate MIME type and extension against an allowlist; store uploads outside the web root.",
    desc: "File uploads are accepted without validating type or extension. An attacker can upload a script and execute it on the server." },
  "weak-crypto":            { label: "Weak Cryptography",         sev: "high",     cwe: "CWE-327",  fix: "Replace MD5/SHA-1 with SHA-256; replace DES/ECB with AES-GCM; use bcrypt ≥12 rounds.",
    desc: "Broken or weak algorithms (MD5, SHA-1, DES, ECB mode) are in use. Hashes are reversible with rainbow tables; ciphertext is trivially decryptable." },
  "prototype-pollution":    { label: "Prototype Pollution",       sev: "high",     cwe: "CWE-1321", fix: "Check that no merge key equals '__proto__', 'constructor', or 'prototype' before deep-merging.",
    desc: "Object properties are merged without filtering dangerous keys. An attacker controlling input keys can corrupt Object.prototype and affect all objects." },
  "watermark-detection":    { label: "AI Watermark",              sev: "high",     fix: "Strip zero-width characters with a pre-commit hook or editor plugin.",
    desc: "Zero-width Unicode characters (U+200B, U+200C, etc.) are embedded in source. These are AI watermarks that can encode model identity or session info." },
  "behavioral-risk":        { label: "Behavioral Risk Pattern",   sev: "high",     cwe: "CWE-506",  fix: "Review and remove the flagged logic; check git blame for when it was introduced.",
    desc: "A logic bomb or data exfiltration pattern was detected — code that activates conditionally based on dates, usernames, or environment values." },
  "open-redirect":          { label: "Open Redirect",             sev: "medium",   cwe: "CWE-601",  fix: "Compare the parsed hostname against an explicit allowlist; reject or strip unknown hosts.",
    desc: "A redirect uses user-supplied input without hostname validation. Attackers can link to your domain while redirecting victims to phishing pages." },
  "weak-cors":              { label: "Weak CORS Policy",          sev: "medium",   cwe: "CWE-942",  fix: "Replace Access-Control-Allow-Origin: * with an explicit origin allowlist.",
    desc: "CORS is set to allow all origins (*). Any website can make credentialed cross-origin requests to this endpoint on behalf of a logged-in user." },
  "insecure-randomness":    { label: "Insecure Randomness",       sev: "medium",   cwe: "CWE-338",  fix: "Use crypto.randomBytes() or secrets.token_bytes() for security-sensitive values.",
    desc: "Math.random() or random.random() is used for a security-sensitive value (token, nonce, ID). These are predictable — use a cryptographically secure RNG." },
  "toctou":                 { label: "TOCTOU Race Condition",     sev: "medium",   cwe: "CWE-367",  fix: "Replace exists()+open() with a single atomic open() in exception-handling mode.",
    desc: "A file existence check is followed by a separate file operation. Between the check and the use, an attacker can swap the file (symlink attack)." },
  "timing-attack":          { label: "Timing Attack",             sev: "medium",   cwe: "CWE-208",  fix: "Use crypto.timingSafeEqual() (Node) or hmac.compare_digest() (Python) for secret comparison.",
    desc: "A secret (token, password) is compared with ===. Response-time differences allow an attacker to guess the secret one byte at a time." },
  "cookie-no-httponly":     { label: "Cookie Missing HttpOnly",   sev: "medium",   cwe: "CWE-1004", fix: "Add HttpOnly to all auth cookies: Set-Cookie: ... HttpOnly; Secure",
    desc: "An auth cookie is set without the HttpOnly flag. JavaScript running on the page (including injected XSS) can read it via document.cookie." },
  "cookie-no-secure":       { label: "Cookie Missing Secure Flag",sev: "low",      cwe: "CWE-614",  fix: "Add the Secure flag to all auth cookies to prevent transmission over plain HTTP.",
    desc: "An auth cookie is set without the Secure flag. The cookie will be sent over unencrypted HTTP connections, exposing the session to network eavesdroppers." },
  "supply-chain-risk":      { label: "Supply Chain Risk",         sev: "medium",   cwe: "CWE-829",  fix: "Verify the package name against the npm/PyPI registry; check for typosquats.",
    desc: "An imported package matches a known typosquat or suspicious dependency pattern. A malicious package could execute code at install or import time." },
  // ── AI attribution signals ─────────────────────────────────────────────────────
  "ai-comment-pattern":      { label: "AI Comment Pattern",                  sev: "low",
    desc: "Comment verbosity and style strongly match AI generation signatures — over-explained, instructional tone, or excessive inline narration of obvious logic." },
  "structural-uniformity":   { label: "Structural Uniformity",               sev: "medium",
    desc: "Code blocks show unusually uniform structure, indentation, and length — typical of AI output generated in a single pass without human variation or refactoring." },
  "comment-density":         { label: "Very High Comment Density",           sev: "low",
    desc: "Comment-to-code ratio far exceeds the human baseline. AI models routinely over-document obvious operations that experienced developers leave uncommented." },
  "identifier-entropy":      { label: "Low Identifier Entropy",              sev: "medium",
    desc: "Variable and function names show low lexical entropy — generic, predictable patterns (data, result, temp, item) that AI models default to without domain context." },
  "style-drift":             { label: "AI Style Drift",                      sev: "low",
    desc: "AI-signal density shifts noticeably between the start and end of the file — indicates a mid-session model switch or a file partially rewritten by an AI tool." },
  "ai-model-attribution":    { label: "AI Model Attribution",                sev: "low",
    desc: "The scanner identified stylistic patterns strongly associated with a specific AI model — see the attribution breakdown for the top candidate and confidence score." },
  "comment-phrasing":        { label: "AI Comment Phrasing",                 sev: "low",
    desc: "Comments use AI-characteristic phrasing — docstrings that open with 'This function…', inline notes that over-explain what the code already says." },
  "language-specific":       { label: "Language-Specific AI Patterns",       sev: "medium",
    desc: "Code uses language-specific idioms that AI models reach for by default — e.g. Python walrus operator, TypeScript strict-null patterns, Go table-driven tests — in density that exceeds human norms." },
  "test-structure":          { label: "Uniform Test Structure",               sev: "low",
    desc: "Test names, assertion counts, and arrange/act/assert block lengths are suspiciously identical across all test cases — typical of AI generating tests in a single batch." },
  "doc-coverage":            { label: "100% Documentation Coverage",          sev: "low",
    desc: "Every exported symbol is documented. Humans routinely leave obvious helpers undocumented; AI generates docs exhaustively by default." },
  "lexical-diversity":       { label: "Low Identifier Diversity (TTR)",       sev: "medium",
    desc: "The same small vocabulary (result, data, handler, item, response…) repeats across all functions. Low type-token ratio is a strong AI signal — humans develop richer domain vocabulary." },
  "error-uniformity":        { label: "Uniform Error Handling",               sev: "low",
    desc: "Every error path uses the exact same try/catch structure and error-message template. Real codebases accumulate inconsistent error handling over time; AI applies one pattern everywhere." },
  "variable-vocabulary":     { label: "Generic AI Variable Vocabulary",       sev: "low",
    desc: "Most variables use generic AI-preferred names (result, data, response, handler, item, error) rather than domain-specific terms that reflect the actual business logic." },
  "sentence-identifiers":    { label: "Sentence-Style Function Names",        sev: "low",
    desc: "Function names average 3+ words in length (processUserAuthentication, validateAndFormatInput). AI prefers verbosely descriptive names; humans tend toward shorter, idiomatic names." },
  "async-consistency":       { label: "Pure Async/Await Consistency",         sev: "low",
    desc: "The file uses async/await throughout with zero .then()/.catch() mixing. Humans typically mix styles across a codebase; AI generates consistent async patterns in one pass." },
  "functional-preference":   { label: "Functional Loop Preference",           sev: "medium",
    desc: "Array transformations use .map/.filter/.reduce/.forEach exclusively — no imperative for or while loops. AI defaults to functional patterns; humans mix both styles." },
  "function-size":           { label: "Uniform Function Sizes",               sev: "low",
    desc: "All functions are suspiciously similar in line count (low coefficient of variation). Organic codebases have wide function-size distributions from years of editing." },
  "nesting-depth":           { label: "Shallow Nesting Depth",                sev: "low",
    desc: "Maximum nesting depth stays at ≤3 levels throughout the file. AI applies guard-clause refactoring consistently; humans accumulate nested callbacks and conditionals over time." },
  "structural-repetition":   { label: "Repetitive Code Structure",            sev: "medium",
    desc: "JSDoc blocks, try/catch wrappers, and line lengths repeat with near-identical structure. Structural repetition at this level indicates a single-pass AI generation, not organic development." },
  "guard-clause-density":    { label: "High Guard Clause Density",            sev: "low",
    desc: "Guard clauses (early returns / precondition checks) are applied uniformly to every function — a best-practice pattern that AI enforces consistently, unlike incrementally-written code." },
  "magic-number-absence":    { label: "Named Constants (no magic numbers)",   sev: "medium",
    desc: "No unexplained numeric literals appear in the code — every value is a named constant. AI always names constants; human code frequently inlines obvious numbers like 0, 1, 100, 1000." },
  "boilerplate":             { label: "High Boilerplate Density",             sev: "low",
    desc: "Null-checks, defensive type assertions, and standard error handlers exceed normal density — AI adds defensive boilerplate everywhere regardless of whether the actual risk warrants it." },
  "template-literals":       { label: "Template Literal Exclusivity",         sev: "medium",
    desc: "Zero string concatenation with +. Every multi-part string uses a template literal. AI applies this consistently in a single pass; humans mix both styles in older code." },
  "method-chain-density":    { label: "High Method Chain Density",            sev: "low",
    desc: "High proportion of method-chain expressions (a.b().c().d()). Fluent interface style is overrepresented compared to human-written code that uses intermediate variables." },
  "naming-consistency":      { label: "Hyper-consistent Naming",              sev: "medium",
    desc: "Naming conventions (camelCase, verb-prefixes, plural collections) are applied without a single exception. Human code always has historical inconsistencies; AI-generated code does not." },
  "import-organization":     { label: "Perfect Import Organisation",          sev: "medium",
    desc: "Imports are perfectly grouped (stdlib → third-party → local), alphabetised within groups, and none are commented out. This level of import hygiene is maintained automatically by AI, not by humans." },
  "dead-code-absence":       { label: "No Dead Code / Debug Artifacts",       sev: "medium",
    desc: "No commented-out code, debug console.logs, informal TODOs, or leftover experimental branches. Human code accumulates these; AI-generated files are surgically clean." },
  "async-try-catch":         { label: "Async Try-Catch Coverage",             sev: "low",
    desc: "Every async function is wrapped in a try/catch block. AI applies this defensively and uniformly; humans often leave happy-path async code unwrapped, especially in early iterations." },
  "immutable-preference":    { label: "Immutable Operation Preference",       sev: "medium",
    desc: "Spread syntax and .map/.filter are used instead of .push/.splice/.sort (which mutate arrays). AI defaults to immutable patterns; humans mix mutable and immutable operations freely." },
  "exhaustive-switches":     { label: "Exhaustive Switch Coverage",           sev: "low",
    desc: "Every switch statement includes a default case. AI generates defensive switches; humans frequently omit the default, especially on familiar enums." },
  "type-guards":             { label: "Type Guards Over Assertions",          sev: "medium",
    desc: "typeof/instanceof predicates are used instead of 'as Type' assertions. AI prefers runtime type-safe patterns; humans often cast aggressively to silence TypeScript errors." },
  "structured-logging":      { label: "Structured Logger Calls",              sev: "low",
    desc: "Logger calls consistently pass structured context objects ({userId, requestId, …}) rather than interpolated strings. AI applies this consistently; human logging accumulates ad hoc format strings." },
  "cyclomatic-uniformity":   { label: "Low + Uniform Cyclomatic Complexity",  sev: "medium",
    desc: "Functions have low, uniform cyclomatic complexity (typically CC 2–6 with low coefficient of variation). AI generates uniformly simple control flow; humans produce a wide complexity distribution." },
  "return-types":            { label: "Return Type Annotation Coverage",       sev: "low",
    desc: "Return types are annotated on all exported and named functions. AI documents types exhaustively by default; humans annotate selectively, especially on obvious internal helpers." },
  "verb-prefix":             { label: "Action-Verb Function Naming",           sev: "low",
    desc: "Function names uniformly follow a verb-prefix pattern (get, set, is, has, create, handle, validate…). AI applies this convention without exception; human naming accumulates historical inconsistencies." },
  "destructuring-density":   { label: "Pervasive Object Destructuring",       sev: "low",
    desc: "Object and array destructuring appear at most assignment sites. AI defaults to destructuring everywhere; humans use property access (obj.prop) where destructuring would be idiomatic." },
  "exception-specificity":   { label: "Custom Typed Exception Usage",         sev: "low",
    desc: "Custom Error subclasses are used instead of bare new Error() or string throws. AI applies domain-specific error hierarchies; humans often throw generic errors, especially in early code." },
  "line-length":             { label: "Tight Line-Length Distribution",        sev: "low",
    desc: "Code line lengths cluster tightly (low coefficient of variation) — no unusually short one-liners or long run-ons. AI formats code to a consistent column width; human code varies widely." },
  "default-params":          { label: "Consistent Default Parameters",         sev: "low",
    desc: "Functions use default parameter syntax instead of manual undefined checks at the top of the body. AI applies this uniformly; humans add defaults as an afterthought or miss them entirely." },
  "arrow-consistency":       { label: "Arrow-Function Callback Exclusivity",   sev: "medium",
    desc: "Exclusively arrow-function callbacks — zero function() callback syntax. AI defaults to arrow functions throughout; human code retains function() in callbacks from older patterns or copy-paste." },
  "ngram-fingerprint":       { label: "AI Token Bigram Fingerprint",           sev: "medium",
    desc: "Characteristic AI token bigrams detected (return { success:, const result = await, throw new Error(`…`), if (!…) return). These phrase patterns appear at 2–4× human baseline frequency." },
  "structural-clones":       { label: "Structural Function Clones",            sev: "medium",
    desc: "3+ function bodies share an identical structural fingerprint (same parameter count, same early-return guard, same return shape). Typical of AI generating CRUD handlers or route controllers in one pass." },
  "error-message-phrasing":  { label: "Templated Error Messages",              sev: "low",
    desc: 'Error messages follow AI templates: "Invalid X", "X is required", "Failed to X", "Unexpected Y". Human error messages accumulate varied phrasing over time.' },
  "identifier-length":       { label: "Uniform Identifier Length Profile",     sev: "low",
    desc: "Identifier lengths cluster in the 7–15 character range with no short loop variables (i, j, k) or very long names. AI generates identifier lengths in a characteristic bell curve." },
  "blank-line-regularity":   { label: "Perfect Blank-Line Discipline",         sev: "medium",
    desc: "Blank lines between functions are exactly 1, with no 3+ consecutive blank lines. AI enforces perfect blank-line discipline; human code accumulates double-blanks and inconsistent spacing." },
  "token-frequency":         { label: "AI Token Frequency Surplus",            sev: "medium",
    desc: "High-frequency AI tokens (await, const, interface, Readonly, Record, Promise, undefined check) appear at 2–4× the human baseline frequency for this language." },
  "prompt-leakage":          { label: "AI Prompt Leakage",                     sev: "high",
    desc: "Fragments resembling AI system-prompt or chat preamble were found in comments or strings — e.g. 'As an AI…', 'Sure! Here is…', 'I cannot…'. Indicates AI output was committed without review." },
  "backdoor-detection":      { label: "Potential Backdoor Pattern",            sev: "high",     cwe: "CWE-506",
    desc: "Suspicious conditional logic activated by specific dates, usernames, or environment values, or covert data exfiltration to an external endpoint. Review immediately." },
  "hallucinated-api":        { label: "Hallucinated / Non-Existent API",       sev: "high",
    desc: "Calls to methods or properties that do not exist in the referenced library (Array.isEmpty, validateAndSave, req.getUserId…). AI confidently invents plausible-sounding APIs." },
  "copy-paste-pattern":      { label: "Copy-Paste / Tutorial Pattern",         sev: "low",
    desc: "Verbatim algorithm implementation matching a common StackOverflow answer or tutorial — sorting, recursion, or data-structure boilerplate that AI lifts from its training corpus." },
};
...
// ── Signal card with line-number chips ───────────────────────────────────────

function LineChip({ line, content }: { line: number; content?: string }) {
  const [show, setShow] = useState(false);
  const sourceLine = content ? (content.split("\n")[line - 1] ?? "").trim() : "";
  return (
    <span className="relative inline-block">
      <span
        onMouseEnter={() => setShow(true)}
        onMouseLeave={() => setShow(false)}
        className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-md bg-gray-100 text-gray-500 text-[10px] font-mono font-bold cursor-default hover:bg-gray-200 transition-colors ring-1 ring-gray-200"
      >
        L{line}
      </span>
      {show && sourceLine && (
        <span
          style={{ pointerEvents: "none", zIndex: 9999, minWidth: 220, maxWidth: 360 }}
          className="absolute bottom-full left-0 mb-1.5 rounded-lg bg-gray-900 text-gray-100 text-[10px] font-mono px-2.5 py-2 shadow-xl whitespace-pre overflow-hidden leading-relaxed"
        >
          {sourceLine.length > 120 ? sourceLine.slice(0, 120) + "…" : sourceLine}
        </span>
      )}
    </span>
  );
...
function SignalCard({ sig, file, layout = "vertical" }: {
  sig: string;
  file: FileResult;
  layout?: "vertical" | "grid";
}) {
  const meta = SIGNAL_META[sig] ?? { label: sig, desc: "Custom detection signal", sev: "medium" as SignalSev };
  const { badge, dot } = SEV_COLORS[meta.sev as SignalSev ?? "medium"];
  const hits = (file.indicators ?? []).filter(i => i.id === sig && i.line);

  return (
    <div className={`flex items-start gap-3 rounded-xl border border-gray-100 p-3 ${layout === "grid" ? "bg-white shadow-sm" : "bg-gray-50"}`}>
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2 mb-0.5 flex-wrap">
          ...
          {"cwe" in meta && meta.cwe && (
            <span className="text-[10px] text-gray-400 font-mono">{meta.cwe}</span>
          )}
        </div>
        <p className="text-[11px] text-gray-500 leading-relaxed">{meta.desc}</p>
        {"fix" in meta && meta.fix && (
          <p className="text-[10px] text-indigo-600 mt-1 leading-relaxed">
            <span className="font-bold">Fix: </span>{meta.fix}
          </p>
        )}
        {hits.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-2">
            {hits.map(h => <LineChip key={h.line} line={h.line!} content={file.content} />)}
          </div>
        )}
      </div>
    </div>
...
function FolderIcon({ size = 13 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-amber-500 shrink-0">
            ...
            <div>
              ...
              <div className="space-y-2">
                {file.risk_indicators.map(sig => (
                    <SignalCard key={sig} sig={sig} file={file} layout="vertical" />
                ))}
              </div>
            </div>
          )}
              ...
              {file.risk_indicators.length > 0 && (
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {file.risk_indicators.map(sig => (
                      <SignalCard key={sig} sig={sig} file={file} layout="grid" />
                    ))}
                  </div>
                </div>
              )}
...
function PRDetailContent() {
  ...
  // Folders collapsed by default; entries here are the ones the user expanded.
  ...
  function markAttested(path: string) {
    ...
    // Also stamp the local scan state so the file row flips to attested immediately
    // and stays that way across re-renders that use scan.files directly.
    setScan(s => s ? {
      ...s,
      files: s.files.map(f => f.file_path === path ? { ...f, attested: true } : f),
    } : null);
    if (scan) {
      resolveOneFile(path);
  ...
  async function attestAll() {
    ...
    for (const f of toAttest) {
      ...
      if (ok) attestedPaths.push(f.file_path);
      setAttestedSet(s => { const n = new Set(s); n.add(f.file_path); return n; });
      recordActivityEvent(f.file_path, email);
    }
    // Stamp attested:true onto local scan state so the view stays correct even
    // if the scan is re-fetched from the API before this session ends.
    setScan(s => s ? {
      ...s,
      files: s.files.map(f => toAttest.some(t => t.file_path === f.file_path)
        ? { ...f, attested: true }
        : f),
    } : null);
    // Mark ALL violations for this scan resolved using snapshot/localStorage
    resolveViolationsForScan(scan.scan_id);
    setAttestingAll(false);
    // Warn if some attestations didn't reach the server (auth/network issue)
    if (attestedPaths.length < toAttest.length && toAttest.length > 0) {
      const failed = toAttest.length - attestedPaths.length;
      console.warn(`[TrustLedger] ${failed} attestation(s) could not be saved to the server. The view is updated locally but will reset on the next hard refresh. Check your reviewer email and network connection.`);
    }
  }
  ...
  // Group files by directory so large file lists are easier to scan.
  const folderGroups = new Map<string, typeof filteredFiles>();
  for (const f of filteredFiles) {
    const dir = f.file_path.includes("/") ? f.file_path.split("/").slice(0, -1).join("/") : "(root)";
    const arr = folderGroups.get(dir);
    if (arr) arr.push(f); else folderGroups.set(dir, [f]);
  }
  const groupedFiles = Array.from(folderGroups.entries()).sort(([a], [b]) => a.localeCompare(b));
          ...
          <div className="animate-fade-up flex items-center gap-3 bg-rose-50 border border-rose-200 rounded-xl px-4 py-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-rose-800">
                {unattested} file{unattested !== 1 ? "s" : ""} in this PR pending attestation
              </p>
              <p className="text-xs text-rose-600 mt-0.5">HIGH and CRITICAL files in this scan require reviewer sign-off before deployment</p>
            </div>
            <button
              onClick={attestAll}
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-50">
                  {groupedFiles.map(([dir, files]) => {
                    const collapsed = !expandedFolders.has(dir);
                    return (
                      <Fragment key={dir}>
                        <tr
                          className="bg-slate-100 hover:bg-slate-200/80 cursor-pointer border-y border-slate-200"
                        ...
                        >
                          <td colSpan={7} className="px-5 py-2">
                            <span className="inline-flex items-center gap-2 text-xs font-bold text-gray-700">
                              <span className={`inline-block text-gray-400 transition-transform duration-150 ${collapsed ? "" : "rotate-90"}`}>›</span>
                              <FolderIcon />
                              <span className="font-mono">{dir === "(root)" ? "/" : `${dir}/`}</span>
                              <span className="text-gray-400 font-semibold normal-case">{files.length} file{files.length !== 1 ? "s" : ""}</span>
                            </span>
                          </td>
                        </tr>
