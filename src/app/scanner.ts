...
function runDetector(
  ...
  for (const re of patterns) {
    for (let i = 0; i < lines.length; i++) {
      ...
      if (opts?.skipComments && (t.startsWith("//") || t.startsWith("#") || t.startsWith("*"))) continue;
...
function findSSRFTainted(lines: string[]): ScanIndicator[] {
  const HTTP_SINK_RE = [
    /(?:fetch|axios|got|needle|superagent|request)\s*\(\s*\w+/i,
...
function sigTypeScript(c: string): number {
  ...
  if (/z\.(?:object|string|number|array|enum|union|literal)\(/.test(c)) s += 0.10;
...
function sigLexicalDiversity(content: string, lineCount: number): number {
  ...
  const ttr    = unique / ids.length;
  // Recalibrated thresholds.
  // Old: fired up to TTR = 0.44, which caught service/repository files with
  //      repetitive method calls (e.g. this.repo.findById × 8 methods).
  // New: only fire on TTR < 0.38, which is genuinely below the human range.
  //   AI code TTR: 0.20–0.32 (heavy identifier reuse across many functions)
  //   Human OOP service files TTR: 0.38–0.50 (repetitive but not AI-level)
  //   Human utility/algorithm files TTR: 0.48–0.65 (high diversity)
...
const LANG_LEAK_BOOST: Partial<Record<string, Partial<Record<string, number>>>> = {
  ...
  javascript: { "cyclomatic-uniformity":0.58 },
...
function computeAIPercentage(
  content: string, lang: string, lineCount: number, priorBias = 0,
  ...
  const combined = Math.min(1.0, Math.max(0,
...
export function analyzeFile(file_path: string, content: string): FileAnalysis {
  ...
  const emptyResult = (): FileAnalysis => ({
    ...
    attribution: { model:"unknown", confidence:0, signals:[], breakdown:{"github-copilot":0,chatgpt:0,gemini:0,claude:0,codewhisperer:0,cursor:0,tabnine:0,human:0,unknown:1} },
  ...
  if (!fileMeta.skipAI) {
    ...
    const { score, fired, applicableCount } = computeAIPercentage(
      content, lang, lineCount, fileMeta.aiPriorBias,
    );
